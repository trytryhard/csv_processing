"""
Module validates files on compatible col-names (content-validation implemented in calc numeric)
"""
...
def cols_validation(path_to_csv: list, needed_cols: list, sep_val: str, enc_val: str) -> list:
    ...
    if False in validated.values():
        to_log.log_message("===Columns validation===\n")
        to_log.log_message(
    ...
    if len([csv_path for csv_path in validated if validated[csv_path]]) == 0:
        ...
        to_log.log_message("There is no files left in queue after validation for processing\n")
